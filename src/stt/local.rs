    ...
    pub async fn prepare(&mut self) -> Result<()> {
        ...
        if !model_path.exists() {
            if self.config.download_models {
                ...
                // Download the model
                download_model(&self.config.model, &model_path).await
                    .with_context(|| format!("Failed to download model: {}", self.config.model))?;
                
                info!("✅ Model downloaded successfully: {:?}", model_path);
            } else {
                let error_msg = format!("Whisper model not found at {:?} and download_models is disabled", model_path);
                warn!("{}", error_msg);
...
async fn download_model(model_name: &str, model_path: &Path) -> Result<()> {
    ...
    let model_file = repo.get(&filename).await
    ...
    tokio::fs::copy(&model_file, &model_path).await
    ...
    info!("✅ Model downloaded successfully: {:.1} MB", metadata.len() as f64 / 1024.0 / 1024.0);
